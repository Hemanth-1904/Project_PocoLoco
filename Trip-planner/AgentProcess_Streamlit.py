...
class StreamToExpander:
    ...
    def write(self, data):
        ...
        if task_value:
            st.success(":robot_face: " + task_value)  # Use Streamlit's success message instead of toast
        ...
        # Repeat the color application for other phrases
        phrases_to_color = [
        ...
        for phrase in phrases_to_color:
            if phrase in cleaned_data:
                cleaned_data = cleaned_data.replace(phrase, 
                                                     f'<span style="color:{self.colors[self.color_index]}">{phrase}</span>')

        # Add cleaned data to the buffer and display when newline is found
        self.buffer.append(cleaned_data)
        if "\n" in data:
            self.expander.markdown(''.join(self.buffer), unsafe_allow_html=True)
            self.buffer = []  # Clear the buffer after displaying
